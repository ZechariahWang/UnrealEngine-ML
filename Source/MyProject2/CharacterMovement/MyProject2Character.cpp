...
{
	// Size the collision capsule.
	GetCapsuleComponent()->InitCapsuleSize(42.f, 96.0f);

	// Let the controller rotate the character only via movement, not directly.
	bUseControllerRotationPitch = false;
	bUseControllerRotationYaw = false;
	bUseControllerRotationRoll = false;

	// Configure character movement: face the direction of travel.
	...
	// Create a camera boom (pulls in toward the character on collision).
	CameraBoom = CreateDefaultSubobject<USpringArmComponent>(TEXT("CameraBoom"));
	CameraBoom->SetupAttachment(RootComponent);
	CameraBoom->TargetArmLength = 400.0f;
	CameraBoom->bUsePawnControlRotation = true;

	// Create the follow camera on the end of the boom.
